angular.module('EditRol', ['ui.bootstrap', 'checklist-model'])
    .controller('EditRolController', ['$scope', '$timeout', '$window', '$uibModal', 'usuario', 'listRoles', 'listPermisos', 'UsuarioService', 'rolData', function ($scope, $timeout, $window, $uibModal, usuario, listRoles, listPermisos, UsuarioService, rolData) {

        $scope.title = 'Editar Rol';
		...
		merge_options($scope.selectedModulos, rolData.data.permisos);

        console.log(rolData.data);
        $scope.rol = {
        	id: rolData.data.rol,
        ...
        $scope.updateRol = function () {
            ...
            UsuarioService.updateRol(data).then(function (result) {                
                if (result.data == 1) {
                    alert("El rol sea ha actualizado correctamente!");
                    $location.path('roles');
                } else {
                    alert("No se ha podido actualizar el Rol");
                }
