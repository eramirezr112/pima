<?php  
    ...
    class DBConfig
    {
        ...
        private function dbConnect()
        {
            ...
            switch (getenv("T_PM_DRVE")) {
                case 'mssqlnative':
                    $db = ADONewConnection(getenv("T_PM_DRVE"));
                    $db->setConnectionParameter('characterSet','UTF-8');
                    $db->setFetchMode(ADODB_FETCH_ASSOC);
                    $db->connect(getenv("T_PM_SN"),getenv("T_PM_UDB"),getenv("T_PM_PDB"),getenv("T_PM_NDB"));
                    break;
                case 'odbc_mssql':
                    $db = ADONewConnection(getenv("T_PM_DRVE"));
                    $db->setConnectionParameter('characterSet','UTF-8');
                    $dsn = "Driver={SQL Server};Server=".getenv("T_PM_SN").";Database=".getenv("T_PM_NDB").";";
                    $db->setFetchMode(ADODB_FETCH_ASSOC);
                    $db->Connect($dsn, getenv("T_PM_UDB"), getenv("T_PM_PDB"));                
                    break;
                default:
                    # code...
                    break;
